...
def plot_maps(images:list, #Image with shape (t, {1,3}, h, w) 
    ...
    cmaps= ['PiYG']*len(images) if cmaps is None else cmaps
    centers= [False]*len(images) if centers is None else centers
    limits= [None]*len(images) if limits is None else limits
    ylabels= ['']*len(images) if ylabels is None else ylabels
